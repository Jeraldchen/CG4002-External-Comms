...
if __name__ == '__main__':
    ...
    eval_to_relay_queue = Queue() # eval to relay
    ...
    shoot_action_queue = Queue() # shoot action
    got_shot_queue = Queue() # got shot
    
    

    # Create processes
    relay_node_server = Process(target=relay_node_server_process, args=(relay_to_ai_queue, eval_to_relay_queue, shoot_action_queue, got_shot_queue, 8800)) # send IMU to AI and send true game state to internal comms
    ...
    eval_client = Process(target=evaluation_client_process, args=(visualiser_to_eval_queue, eval_to_visualiser_queue, eval_to_relay_queue, 'localhost', eval_server_port)) # send true game state to eval server
